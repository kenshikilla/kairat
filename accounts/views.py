...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Ссылка переактивации вышлена на вашу почту')
            return redirect('login')
        else:
            messages.error(request, 'Аккаунта не существует')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Сбросьте ваш пароль')
        return redirect('resetPassword')
    else:
        messages.error(request, 'Время на ссылку истекло')
        return redirect('login')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Пожалуйста введите верный пароль')
                return redirect('change_password')
        else:
